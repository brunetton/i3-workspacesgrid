...
if args['print-i3-conf']:
    ...
    command = "cat {} | sed -n -r 's|bindsym \$mod\+([^ ]+) workspace ([0-9]+)\s*|bindsym \$mod\+\\1 exec curl http://localhost:{}/jump/\\2|p'"
    command = command.format(I3_CONFIG_FILE, conf.getint('server', 'port'))
    try_to_run(command)
    print "# Direct sending containers to workspaces"
    command = "cat {} | sed -n -r 's|bindsym \$mod\+([^ ]+) move.+workspace ([0-9]+)\s*|bindsym \$mod\+\\1 exec curl http://localhost:{}/send/\\2|p'"
    command = command.format(I3_CONFIG_FILE, conf.getint('server', 'port'))
    try_to_run(command)
    # New commands
